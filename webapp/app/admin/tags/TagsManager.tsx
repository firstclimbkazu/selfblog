              ...
              {tags.map((tag, i) => (
                <tr key={tag.id} className={i % 2 === 0 ? "" : "bg-[var(--sotw-surface-alt)]"}>
                  ...
                  <td className="px-4 py-3 text-right space-x-4 whitespace-nowrap">
                    <Link href={`/admin/tags/${tag.id}/edit`} className="text-xs text-[var(--sotw-moss)] hover:underline">
                      編集
                    </Link>
                    <InlineDeleteButton label={tag.name} action={deleteTag.bind(null, tag.id)} />
                  </td>
                </tr>
              ))}
            </tbody>
